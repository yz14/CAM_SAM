"""XGradCAM 教学 demo（独立可运行）。

与 GradCAM 的差异（重点）
-------------------------
论文 *Axiom-based Grad-CAM (NeurIPS 2020)* 证明 GradCAM 不满足 "Sensitivity / Conservation"
两条公理。XGradCAM 将通道权重改为：
    w_k = sum( A_k * dY/dA_k ) / sum(A_k)
即 **以激活值作为归一化分母**，从而满足类别一致性公理。直观效果：热力图与类别
关系更紧，对相似类别更可分辨。

API 与 GradCAM 完全一致。

运行：
    D:\\miniconda\\envs\\py310\\python.exe examples/demo_xgradcam.py
"""
...
OUT = Path(__file__).resolve().parent.parent / "outputs" / "demo_xgradcam.png"
...
def main() -> None:
    ...
    print(f"[demo_xgradcam] predicted class = {pred}")
    ...
    print(f"[demo_xgradcam] saved -> {OUT}")
