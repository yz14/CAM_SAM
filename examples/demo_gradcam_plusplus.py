"""GradCAM++ 教学 demo（独立可运行）。

与 GradCAM 的差异（重点）
-------------------------
GradCAM 把"梯度对 H×W 取全局平均"作为通道权重，遇到 **图像中同一类有多个实例**
或目标尺寸差异较大时容易丢掉小目标。
GradCAM++ 把通道权重改为"梯度的高阶项加权平均"（论文 Eq.19），等价于对
不同空间位置赋予不同重要性，因此 **多目标 / 小目标定位更稳**。

API 上与 GradCAM **完全一致**：只是把 `GradCAM` 换成 `GradCAMPlusPlus`。
...
OUT = Path(__file__).resolve().parent.parent / "outputs" / "demo_gradcam_plusplus.png"
...
def main() -> None:
    ...
    print(f"[demo_gradcam_plusplus] predicted class = {pred}")

    target_layers = [model.layer4[-1]]
    # ★ 唯一改动：GradCAM -> GradCAMPlusPlus
    ...
    print(f"[demo_gradcam_plusplus] saved -> {OUT}")
